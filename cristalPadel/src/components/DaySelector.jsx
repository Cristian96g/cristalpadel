...
export default function DaySelector({ selectedDate, onSelectDate }) {
  const safeDate = selectedDate || formatISODate(new Date());

  const dates = getVisibleDates(safeDate);
  const monthLabel = getMonthLabel(safeDate);
  const todayText = getTodayText(safeDate);

  function moveDay(amount) {
    onSelectDate(addDays(safeDate, amount));
  }
